...
\begin{align}
 P_1(q|\vec{\theta}) &= \begin{cases}
     ...
     \end{cases} \\
 P_2(q|\vec{\theta}) &= \frac{A}{q \sqrt{2\pi\sigma^2}} e^{-\frac{(\ln{q} - \mu)^2}{2\sigma^2}}
\end{align}
...
\begin{equation}
P_1(\vec{\theta} | \{\vec{x_k}\}) \propto P_1(q|\vec{\theta}) P(\vec{\theta}| \alpha, m, \tau, \epsilon) P(\alpha, m, \tau, \epsilon)
\end{equation}
...
We show the resulting distribution in Figure \ref{fig:mrd}. The $1\sigma$ uncertainties in the bin heights from the histogram model are shown as error bars, and we overplot 500 samples of the log-normal fit to show the spread allowed by the data. 
